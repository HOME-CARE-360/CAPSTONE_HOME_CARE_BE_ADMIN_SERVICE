...
class PDFUtils {
    static drawTable(
        ...
        data.forEach((row, rowIndex) => {
            ...
            if (isHeader) {
                doc.rect(x, y + (rowIndex * rowHeight), width, rowHeight)
            ...
            row.forEach((cell, colIndex) => {
                ...
                doc.fontSize(PDF_CONFIG.FONT_SIZES.SMALL)
                    .font(isHeader ? PDF_CONFIG.FONTS.BOLD : PDF_CONFIG.FONTS.REGULAR)
                    .fillColor(isHeader ? PDF_CONFIG.COLORS.PRIMARY : PDF_CONFIG.COLORS.SECONDARY)
                    .text(
                        cell || '',
                        cellX + 5,
                        cellY + (rowHeight / 2) - 6,
            ...
            for (let i = 1; i < row.length; i++) {
                doc.moveTo(x + (i * colWidth), y + (rowIndex * rowHeight))
    ...
    static addSectionTitle(doc: PDFKit.PDFDocument, title: string, yPosition: number): number {
        doc.fontSize(PDF_CONFIG.FONT_SIZES.SECTION)
            .font(PDF_CONFIG.FONTS.BOLD)
            .text(title, PDF_CONFIG.MARGIN, yPosition);
        return yPosition + 30;
    }

    static addSubsectionTitle(doc: PDFKit.PDFDocument, title: string, yPosition: number): number {
        doc.fontSize(PDF_CONFIG.FONT_SIZES.SUBSECTION)
            .font(PDF_CONFIG.FONTS.BOLD)
            .text(title, PDF_CONFIG.MARGIN, yPosition);
        return yPosition + 25;
    }

    static checkPageBreak(doc: PDFKit.PDFDocument, yPosition: number, requiredSpace: number): number {
...
class DatabaseQueries {
    ...
    async getUserStatistics(startDate: Date, endDate: Date): Promise<UserStatistics> {
        const [
            totalUsers,
            newUsers,
            activeUsers,
    ...
    async getRoleStatistics(endDate: Date): Promise<RoleStatistics> {
        ...
        const roles = rolesWithUsers.map(role => ({
            ...
            percentage: totalUsers > 0 ? Math.round((role._count.User_UserRoles / totalUsers) * 100) : 0
        }));
    ...
    async getTopUsers(startDate: Date, endDate: Date, limit: number = 10): Promise<TopUser[]> {
    ...
    async getActivitySummary(startDate: Date, endDate: Date): Promise<ActivitySummary> {
        const [totalDevices, totalNotifications, mostActiveUsers] = await Promise.all([
...
class PDFContentGenerator {
    static generateHeader(doc: PDFKit.PDFDocument, title: string): number {
        doc.fontSize(PDF_CONFIG.FONT_SIZES.TITLE)
            .font(PDF_CONFIG.FONTS.BOLD)
            .text(title, PDF_CONFIG.MARGIN, 50, { align: 'center' });

        doc.fontSize(PDF_CONFIG.FONT_SIZES.BODY)
            .font(PDF_CONFIG.FONTS.REGULAR)
            .text(`Generated on: ${DateUtils.formatCurrentDateTime()}`, PDF_CONFIG.MARGIN, 80, { align: 'center' });

        // Draw horizontal line
        doc.moveTo(PDF_CONFIG.MARGIN, 110)
            .lineTo(550, 110)
            .stroke();
    ...
    static generateUserStatisticsSection(doc: PDFKit.PDFDocument, data: MonthlyReportData, yPosition: number): number {
        ...
        const userStats = [
            ...
            ['Deleted Users This Month', data.userStatistics.deletedUsers.toLocaleString()]
        ];
        ...
        // User type breakdown
        yPosition = PDFUtils.addSubsectionTitle(doc, 'User Type Breakdown:', yPosition);
        ...
        PDFUtils.drawTable(doc, userTypeStats, PDF_CONFIG.MARGIN, yPosition, 500, 25);
        return yPosition + userTypeStats.length * 25 + 30;
    }

    static generateRoleStatisticsSection(doc: PDFKit.PDFDocument, data: MonthlyReportData, yPosition: number): number {
        yPosition = PDFUtils.checkPageBreak(doc, yPosition, 200);
        yPosition = PDFUtils.addSectionTitle(doc, '2. ROLE STATISTICS', yPosition);

        doc.fontSize(PDF_CONFIG.FONT_SIZES.BODY)
            .font(PDF_CONFIG.FONTS.REGULAR)
            .text(`Total Roles: ${data.roleStatistics.totalRoles}`, PDF_CONFIG.MARGIN, yPosition);
        ...
        if (data.roleStatistics.roles.length > 0) {
            ...
            PDFUtils.drawTable(doc, [roleHeaders, ...roleData], PDF_CONFIG.MARGIN, yPosition, 500, 25, true);
    ...
    static generateTopUsersSection(doc: PDFKit.PDFDocument, data: MonthlyReportData, yPosition: number): number {
        yPosition = PDFUtils.checkPageBreak(doc, yPosition, 250);
        yPosition = PDFUtils.addSectionTitle(doc, '3. NEW USERS THIS MONTH', yPosition);
        ...
        if (data.topUsers.length > 0) {
            ...
            PDFUtils.drawTable(doc, [userHeaders, ...userData], PDF_CONFIG.MARGIN, yPosition, 500, 25, true);
    ...
    static generateActivitySection(doc: PDFKit.PDFDocument, data: MonthlyReportData, yPosition: number): number {
        yPosition = PDFUtils.checkPageBreak(doc, yPosition, 250);
        ...
        const activityStats = [
            ...
            ['Total Notifications', data.activitySummary.totalNotifications.toLocaleString()]
        ];

        PDFUtils.drawTable(doc, activityStats, PDF_CONFIG.MARGIN, yPosition, 500, 25);
        yPosition += activityStats.length * 25 + 30;

        // Most active users
        if (data.activitySummary.mostActiveUsers.length > 0) {
            yPosition = PDFUtils.addSubsectionTitle(doc, 'Most Active Users:', yPosition);

            const activeUserHeaders = ['Name', 'Email', 'Login Count'];
            const activeUserData = data.activitySummary.mostActiveUsers.map(user => [
                user.name,
                user.email,
                user.loginCount.toString()
            ]);

            PDFUtils.drawTable(doc, [activeUserHeaders, ...activeUserData], PDF_CONFIG.MARGIN, yPosition, 500, 25, true);
    ...
    static generateFooter(doc: PDFKit.PDFDocument): void {
        doc.fontSize(PDF_CONFIG.FONT_SIZES.SMALL)
            .font(PDF_CONFIG.FONTS.REGULAR)
            .text('Report generated automatically by Admin System', PDF_CONFIG.MARGIN, doc.page.height - 70, { align: 'center' });
    }
}
...
export class ReportRepository {
    ...
    async getMonthlyReportData(month: number, year: number): Promise<MonthlyReportData> {
        ...
        try {
            ...
            throw new AppError(
                ...
                { month, year, error: error instanceof Error ? error.message : 'Unknown error' },
                500
            );
        }
    }
    ...
    async exportMultipleMonthsReportPDF(
        ...
        try {
            const reports = await this.getMultipleMonthsData(startMonth, startYear, endMonth, endYear);
            ...
            return new Promise((resolve, reject) => {
                ...
                this.generateMultiMonthPDFContent(doc, reports, startMonth, startYear, endMonth, endYear);
                doc.end();
            });
            ...
            throw new AppError(
                ...
                { startMonth, startYear, endMonth, endYear, error: error instanceof Error ? error.message : 'Unknown error' },
                500
            );
        }
    }
    ...
    private async getMultipleMonthsData(
        ...
        while (currentYear < endYear || (currentYear === endYear && currentMonth <= endMonth)) {
            const monthData = await this.getMonthlyReportData(currentMonth, currentYear);
    ...
    private generateSingleMonthPDFContent(doc: PDFKit.PDFDocument, data: MonthlyReportData): void {
        ...
        yPosition = PDFContentGenerator.generateUserStatisticsSection(doc, data, yPosition);
        yPosition = PDFContentGenerator.generateRoleStatisticsSection(doc, data, yPosition);
        yPosition = PDFContentGenerator.generateTopUsersSection(doc, data, yPosition);
        yPosition = PDFContentGenerator.generateActivitySection(doc, data, yPosition);
    ...
    private generateMultiMonthPDFContent(
        ...
        // Summary table
        const summaryHeaders = ['Month/Year', 'New Users', 'Total Users', 'Active Users', 'Logins'];
        ...
        PDFUtils.drawTable(doc, [summaryHeaders, ...summaryData], PDF_CONFIG.MARGIN, yPosition, 500, 25, true);
