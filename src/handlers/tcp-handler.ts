...
export async function handleTCPRequest(
  payload: TCPPayload,
): Promise<HandleTCPReturn> {
...
async function handleUpdateUser(data: any): Promise<HandlerResult> {
  if (!data?.id || !data?.adminId) {
    throw new AppError(
      "Error.MissingUserId",
      [{ message: "User ID and adminId are required", path: ["id"] }],
      {},
      400,
    );
  }
...
async function handleResetPassword(data: any): Promise<HandlerResult> {
  ...
  const user = await service.resetUserPassword(
    parsed.id,
    parsed.newPassword,
    parsed.adminId,
  );
...
async function handleAssignRoles(data: any): Promise<HandlerResult> {
  const parsed = parseWithSchema(AssignRolesSchema, data) as AssignRolesDTO & {
    adminId: number;
  };
...
async function handleAssignPermissionsToRole(
  data: any,
): Promise<HandlerResult> {
  const parsed = parseWithSchema(
    AssignPermissionsToRoleSchema.extend({
      adminId: z.number().int().positive(),
    }),
    data,
...
async function handleExportMonthlyPDF(data: any): Promise<HandlerResult> {
  ...
  const buffer = await service.exportMonthlyReportPDF(
    parsed.month,
    parsed.year,
  );
  return {
